...
void cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
 ...
 // for debugging abd visualizing both rows together
  ...
  if (coefficients_row1_plane->values[1] >-0.9 || coefficients_row2_plane->values[1] > -0.9)
  {
    ROS_WARN_STREAM("Detected rows are outside range for row-following!");
    ROS_WARN_STREAM("coefficient 1 is " << coefficients_row1_plane->values[1] <<" coefficient 2 is " << coefficients_row2_plane->values[1]);
    driveStraight();
    return;
  }
